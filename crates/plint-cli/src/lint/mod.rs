...
pub(super) fn load_ruleset(ruleset_name: &str) -> Vec<Result<Ruleset, RulesetLoadError>> {
    ...
    match std::fs::read_to_string(index_path) {
        Ok(index_string) => match yaml_serde::from_str::<IndexFile>(&index_string) {
            ...
            Err(err) => {
                vec![Err(RulesetLoadError::IndexParseError(err))]
            }
        },
        Err(err) => {
            vec![Err(RulesetLoadError::IndexReadError(err))]
        }
    }
}

pub(super) fn load_ruleset_with_index(
    ruleset_name: &str,
    index: &IndexFile,
        ...
        match std::fs::read_to_string(group_path) {
            Ok(group_string) => match yaml_serde::from_str::<Group>(&group_string) {
                Ok(group) => group
                    .rulesets
                    .iter()
                    .map(|ruleset_name| load_ruleset_with_index(ruleset_name, index))
                    .flatten()
                    .collect(),
